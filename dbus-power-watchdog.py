...
"""
D-Bus grid meter service for Venus OS that reads AC line data from a
Hughes Power Watchdog surge protector via Bluetooth Low Energy.

Publishes as com.victronenergy.grid (or genset/pvinverter depending on
configured role) so that Venus OS treats the data as an AC input,
visible in VRM and the GX console.
...
class PowerWatchdogService:
    """Venus OS D-Bus grid meter service backed by a Power Watchdog BLE connection."""

    def __init__(self):
        config = load_config()
        defaults = config["DEFAULT"]

        self._mac_address = defaults.get("mac_address", "").strip()
        if not self._mac_address:
            logger.error("No mac_address configured!")
            sys.exit(1)

        adapter = defaults.get("bluetooth_adapter", "").strip()
        self._update_interval = int(defaults.get("update_interval", "5"))
        reconnect_delay = float(defaults.get("reconnect_delay", "10"))
        reconnect_max_delay = float(defaults.get("reconnect_max_delay", "120"))
        ...
        settings_path = "/Settings/Devices/power_watchdog_%s" % (
            self._mac_address.replace(":", "").lower()
        )
    ...
    def _create_service(self, role: str):
        ...
        if self._dbusservice is not None:
            ...
            if self._timer_id is not None:
                ...
                self._timer_id = None
            # __del__ on VeDbusService unregisters from D-Bus
        ...
        servicename = "%s.%s" % (service_class, SERVICE_SUFFIX)
        ...
        self._dbusservice.add_path("/HardwareVersion", "50A BLE")
    ...
    def _on_role_changed(self, path, value):
        ...
        # Schedule re-creation on next mainloop iteration (can't do it
        # inside the D-Bus callback that owns the service being destroyed)
    ...
    def _handle_setting_changed(self, setting, oldvalue, newvalue):
        """Called when a setting is changed externally (e.g. from another process)."""
    ...
    def _update(self) -> bool:
        ...
        if data.timestamp > 0:
            ...
            if data.has_l2:
                ...
                # Use highest error code from either line
            ...
            # Use L1 frequency as the global frequency
            if l1.frequency > 0:
